...
function(record, search, email, runtime, format) {
    ...
    function invcProcess(internalid){
    	...
    	var ticketnum = invcSearch.getValue({ fieldId:'custbody_spos_posticketnumber'});
        ...
        if(loyaltyFound == true){
        	...
        	log.debug('Invoice record updated [UPDATE]', recordsubmit);
        }
        	...
        	log.debug('Invoice record updated [NO LOYALTY ID]', recordsubmit);
        }
        	...
        	log.debug('Invoice record updated [LOYALTY ID NOT FOUND]', recordsubmit);
        }
    }
});
